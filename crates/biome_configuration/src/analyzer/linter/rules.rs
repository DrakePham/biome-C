...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub use_mutation_input_name:
        Option<RuleConfiguration<biome_graphql_analyze::options::UseMutationInputName>>,
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.use_mutation_input_name.as_ref() {
            if rule.is_enabled() {
    ...
    pub(crate) fn get_rule_configuration(
        ...
        match rule_name {
            ...
            "useImportRestrictions" => self
                ...
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "useMutationInputName" => self
                .use_mutation_input_name
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
